...
test("adjacent passive partitions violate the configured gap", async () => {
  ...
  const verticalGap =
    c1Placement.y - c1Size.y / 2 - (c3Placement.y + c3Size.y / 2)

  expect(verticalGap).toBeLessThan(inputProblem.partitionGap)
