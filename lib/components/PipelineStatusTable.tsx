...
export const PipelineStatusTable = ({ solver, runCount }: PipelineStatusTableProps) => {
  const stageNames = [
  ...
  const getStageInfo = (): PipelineStageInfo[] => {
    return solver.pipelineDef.map((stepDef, index) => {
      ...
      const firstIterationStarted = solver.firstIterationOfPhase[stepDef.solverName] ?? null
      ...
      const downloadInputsData = () => {
        try {
          ...
          console.error(`Failed to get constructor params for ${stepDef.solverName}:`, error)
          return []
        }
      }
          ...
          {stageInfos.map((stageInfo, index) => (
            <tr key={index}>
              ...
              <td style={{ 
                border: '1px solid #ccc', 
                padding: '8px',
                color: stageInfo.status === 'Completed' ? 'green' : 
                       stageInfo.status === 'In Progress' ? 'orange' : 
                       'gray'
              }}>
                {stageInfo.status}
              </td>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>
                {stageInfo.firstIterationStarted !== null ? stageInfo.firstIterationStarted : '-'}
              </td>
              <td style={{ border: '1px solid #ccc', padding: '8px' }}>
                <button
