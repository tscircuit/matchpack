...
export class OverlapResolutionSolver extends BaseSolver {
  ...
  constructor(inputLayout: OutputLayout, chipMap?: Record<string, { size: { x: number; y: number } }>) {
    super()
  ...
  override _step() {
    // Simple bounding box overlap resolution
  ...
  const minGap = 0.2
    ...
    for (let i = 0; i < chipIds.length; i++) {
      for (let j = i + 1; j < chipIds.length; j++) {
        ...
        if (!a || !b) continue;
        // Use actual chip sizes if available
