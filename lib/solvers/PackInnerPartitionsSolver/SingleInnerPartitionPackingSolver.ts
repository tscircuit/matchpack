  ...
  private createPackInput(): PackInput {
    ...
    for (const [connKey, connected] of Object.entries(
      ...
      if (pins.length === 2 && pins[0] && pins[1]) {
        const pin1 = this.inputProblem.chipPinMap[pins[0]]
        const pin2 = this.inputProblem.chipPinMap[pins[1]]
    ...
    for (const [connKey, connected] of Object.entries(
      ...
      if (pinId && netId) {
          ...
          if (fromChip === chipId) {
            ...
            for (const [otherConnKey, otherConnected] of Object.entries(
              ...
              if (otherNetId === netId && otherPinId && otherPinId !== pinId) {
                const otherPin = this.inputProblem.chipPinMap[otherPinId]
        ...
        if (shouldIncludeInNetwork) {
          ...
          pinToNetworkMap.set(
            pinId,
            `${pinId}_opposite-strong-side-disconnected`,
          )
        }
      }
    }
